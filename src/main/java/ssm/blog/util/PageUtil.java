...
public class PageUtil {
	 ...
	 * @return
	 */
	public static String genPagination(String targetUrl, // 目标url
			long totalNum, // 总记录数
			int currentPage, // 当前页
	...
	public static String getPrevAndNextPageCode(Blog prev, Blog next, String projectContent) {
		...
		if (prev == null || prev.getId() == null) {
			...
			pageCode.append("<p>上一篇：<a href='" + projectContent + "/blog/articles/" + prev.getId() + ".html'>"
					+ prev.getTitle() + "</a></p>");
		}
		...
		if (next == null || next.getId() == null) {
			...
			pageCode.append("<p>上一篇：<a href='" + projectContent + "/blog/articles/" + next.getId() + ".html'>"
					+ next.getTitle() + "</a></p>");
		}
	...
	public static String getUpAndDownPageCode(Integer page, Integer totalNum, String q, Integer pageSize,
		...
		if (totalPage == 0) {
			...
			if (page > 1) {
				pageCode.append("<li><a href='" + projectContext + "/blog/search.html?page=" + (page - 1) + "&q=" + q
						+ "'>上一页</a></li>");
			} else {
				pageCode.append("<li class='disabled'><a>上一页</a></li>");
			}
